...
from illustrator_ops import open_and_color_template, place_and_simulate_print, update_size_label, delete_side_assets, save_pdf
...
def process_order(order):
    ...
    try:
        ...
        # שמירה ישירות בתיקיית ההזמנה (ללא תאריך)
        folder = os.path.join(ORDERS_ROOT_DIR, oid)
        ...
        try:
            # קבלת צבע החולצה (חשוב לחישוב הקונטרסט)
        ...
        # שמירת PDF
        base_pdf_path = os.path.join(folder, f"{oid}.pdf")
...
# --- נקודת הכניסה (מעודכן לתמיכה ב-PDF) ---
if __name__ == "__main__":    
    if len(sys.argv) > 1:
        try:
            ...
            print(f"Received order data for: {order_data.get('order_id')}")
            ...
            for loc in ['front', 'back', 'right_sleeve', 'left_sleeve']:
                ...
                if loc_data.get('exists'):
                    ...
                    if url:
                        ...
                        # שם קובץ לשמירה במחשב
                        filename_prefix = f"{order_data['order_id']}_{loc}"
